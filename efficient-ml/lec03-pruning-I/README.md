...
给定原始权重 $W$，剪枝可以表述为带 $\ell_0$ 约束的优化问题：

$$\arg\min_{W_P} \mathcal{L}(x;\, W_P) \quad \text{s.t.} \quad \|W_P\|_0 \leq N$$

其中 $\|W_P\|_0$ 是非零权重数，$N$ 是参数预算。这是一个 NP-hard 问题（$\ell_0$ 约束不可微、不凸），实际中只能用启发式方法逼近。
...
对于卷积层权重 $[C_{out},\, C_{in},\, k_H,\, k_W]$，常见的粒度从细到粗依次为：
...
| **Kernel-level** | 一个 $k_H \times k_W$ kernel | 较规则 | — |
...
- 按 filter（L1-norm）：$\|\mathbf{f}\|_1 = \sum_i |w_i|$
- 按 filter（L2-norm）：$\|\mathbf{f}\|_2 = \sqrt{\sum_i w_i^2}$

**示例**：对权重矩阵 $W = \begin{bmatrix} 3 & -2 \\ 1 & -5 \end{bmatrix}$ 做 50% 非结构化剪枝。按绝对值排序为 5, 3, 2, 1，删最小的两个（2 和 1），得 $\begin{bmatrix} 3 & 0 \\ 0 & -5 \end{bmatrix}$。

按 L1-norm 做行级结构化剪枝：第 0 行 $|3|+|-2|=5$，第 1 行 $|1|+|-5|=6$，删 L1 更小的第 0 行，得 $\begin{bmatrix} 0 & 0 \\ 1 & -5 \end{bmatrix}$。
...
引入掩码变量 $c_j \in \{0,1\}$，用一阶 Taylor 展开近似 loss 变化，经链式法则推导出 saliency 分数：
...
其中 $h_{ii} = \frac{\partial^2 \mathcal{L}}{\partial w_i^2}$ 是 Hessian 对角元素。

**OBS（Optimal Brain Surgeon）**（Hassibi & Stork, NeurIPS 1993）放松了对角假设，使用完整 Hessian 逆，而且能给出删除某权重后剩余权重的最优补偿调整量（weight update）。理论上更精确，但计算 Hessian 逆的复杂度是 $O(n^2) \sim O(n^3)$，对大模型完全不现实。
...
不从全局 loss 出发，而是**逐层最小化特征重建误差**。设第 $l$ 层输出 $Z = XW^T$，引入通道选择向量 $\beta$：

$$\arg\min_{W,\,\beta} \left\| Z - \sum_{c=0}^{C_{in}-1} \beta_c\, X_c\, W_c^T \right\|_F^2 \quad \text{s.t.} \quad \|\beta\|_0 \leq N_c$$

其中 $\beta_c = 0$ 表示第 $c$ 个 input channel 被剪掉。通过交替优化（固定 $W$ 解 $\beta$，固定 $\beta$ 解 $W$）找到对下游特征影响最小的剪枝方案。
...
|:---|:---|:---:|:---:|:---|:---|
| Magnitude | $\|w\|_p$ | 否 | 否 | 极低 | 快速基线、资源受限 |
| SNIP | $\|g \cdot w\|$ | 1 batch | 1 次反向 | 低 | 训练前剪枝 |
| OBD | $\frac{1}{2} h_{ii} w_i^2$ | 是 | 二阶 | 高 | 小模型精剪 |
| APoZ | 零激活比例 | 是 | 否 | 中 | Channel pruning（ReLU 网络） |
| Reconstruction | $\min\|Z - \hat{Z}\|_F^2$ | 是 | 否 | 中 | 逐层精细剪枝 |
...
只看绝对值大小，忽略了权重对 loss 的实际敏感度。一个绝对值小的权重可能恰好位于决策边界的关键位置。SNIP 的 $|g \cdot w|$ 同时考虑梯度和权重，更合理；OBD 更进一步引入了二阶 Hessian 信息。
