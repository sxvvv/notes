...
$$
\mathrm{params} = C_{in} \times C_{out} + C_{out} \quad (\text{bias})
$$
...
$$
\mathrm{params} = C_{out} \times C_{in} \times K_H \times K_W + C_{out} \quad (\text{bias})
$$
...
| 投影 | 形状 |
|------|------|
| $W_Q, W_K, W_V$ | $d_{model} \times d_{model}$ |
| $W_O$ | $d_{model} \times d_{model}$ |

忽略 bias 时总参数量：$4 \times d_{model}^2$。

GPT-2 small（$d_{model} = 768$）单层 attention 参数 $= 4 \times 768^2 \approx 2.36\text{M}$。但还有 FFN（通常参数比 attention 更多），下文会拆解。
...
$$
\mathrm{MACs} = B \times C_{in} \times C_{out}
$$

每个输出元素需要 $C_{in}$ 次 multiply-accumulate。

### Conv2d 层

$$
\mathrm{MACs} = B \times C_{in} \times C_{out} \times K_H \times K_W \times H_{out} \times W_{out}
$$

与参数量相比，多出 $H_{out} \times W_{out}$。这就是参数量与计算量能"脱钩"的原因——$3 \times 3$ 卷积可能只有 81 个参数，但在 $1000 \times 1000$ 的 feature map 上做一次 forward，MACs 就有约 81M。
...
$$
\underbrace{4Nd^2}_{QKV + O \text{ 投影}} + \underbrace{2N^2 d}_{\text{注意力矩阵 } (QK^T + \mathrm{score} \cdot V)}
$$
...
$$
\mathrm{Memory_{train}} = \text{权重} + \text{梯度} + \text{Optimizer States} + \text{Activations}
$$
...
$$
\mathrm{value} = (-1)^{sign} \times 2^{(exponent - bias)} \times (1 + mantissa)
$$
...
$$
H_{out} = \lfloor (224 + 2 \times 3 - 7) / 2 + 1 \rfloor = 112
$$

MACs：

$$
\mathrm{MACs} = 64 \times 3 \times 7 \times 7 \times 112 \times 112 \approx 118\text{M}
$$
...
$d_{model} = 768$，$d_{ff} = 3072 = 4 \times d_{model}$。
...
- **Embedding 的特殊性**：Embedding 参数量可以很大（$\mathrm{vocab\\_size} \times d_{model}$），但"计算"只是一次 gather 操作，FLOPs 贡献微乎其微。
...
- Embedding 层参数量巨大（$\mathrm{vocab\\_size} \times d_{model}$），但 MACs 几乎为零——仅做一次 gather 查表。
...
$$
\mathrm{KV\\_cache} = 2 \times L \times n_{kv\\_heads} \times d_{head} \times N \times B \times \mathrm{dtype\\_bytes}
$$
