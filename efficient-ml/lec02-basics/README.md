...
GPT-2 small 是由 12 层 Transformer Block 组成的 Decoder-only 结构，$d_{model} = 768$，$d_{ff} = 3072 = 4 \times d_{model}$。
...
| Attention：$W_{QKV}$ $(768 \times 768 \times 3 + 768 \times 3)$ + $W_{proj}$ $(768 \times 768 + 768)$ | 2,362,368 |
...
反过来，Embedding 层参数量巨大（$\mathrm{vocab\_size} \times d_{model}$），但计算约等于零——只是一次 gather 查表。
...
输出分辨率：$H_{out} = \lfloor (224 + 2 \times 3 - 7) / 2 + 1 \rfloor = 112$
...
- **数据侧：** SFT 需要额外存 loss_mask（标记哪些 token 参与 loss 计算），pretrain 通常不需要。数据集占内存会稍大。
