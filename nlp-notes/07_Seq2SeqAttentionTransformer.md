...
Bahdanau et al. (2014) 提出的注意力机制做了一个关键改动：解码器在每个时间步都能访问编码器的**所有**隐藏状态，而非仅最后一个。

### 2.2 计算流程（交叉注意力）

以解码器在时间步 t 生成词为例：

1. **计算注意力分数**：用解码器当前状态与编码器每个隐藏状态计算相关性分数
2. **归一化**：对分数做 Softmax 得到注意力权重
3. **加权求和**：用权重对编码器隐藏状态求和，得到上下文向量
4. **融合输出**：将上下文向量与解码器状态拼接后送入输出层

关键点：每个时间步的上下文向量是**不同的**——翻译"天气"时关注源序列中的"天气"，翻译"good"时关注"好"。这就是"动态对齐"。
...
$$
\mathrm{Attention}(Q, K, V) = \mathrm{softmax}\!\left(\frac{QK^\top}{\sqrt{d\_k}}\right)V
$$
...
1. 对输入 X 分别乘以三个可学习的权重矩阵，得到 Q、K、V：
...
$$
Z = \mathrm{softmax}\!\left(\frac{QK^\top}{\sqrt{d\_k}}\right)V
$$

其中 QK 转置相乘的结果是一个 `[seq_len, seq_len]` 的矩阵，第 i 行第 j 列表示第 i 个词对第 j 个词的关注程度。整个计算是矩阵乘法，可以完全并行。

**为什么需要三个独立矩阵而不是直接用 X？** 因为"查询"、"索引"、"内容"在信息检索中承担不同角色。三个权重矩阵让模型学会把同一个输入投影到三个功能不同的空间中。
...
单组权重矩阵只能从一个"视角"捕获关系。多头注意力并行运行 h 组独立的注意力计算，每组关注不同类型的依赖（语法、语义、指代等），最后拼接结果并投影：

$$
\mathrm{MultiHead}(Q,K,V) = \mathrm{Concat}(\mathrm{head}\_1, \dots, \mathrm{head}\_h) \cdot W^O
$$
...
$$
\mathrm{FFN}(x) = \mathrm{ReLU}(x W\_1 + b\_1) W\_2 + b\_2
$$

第一个权重矩阵将维度从 d 升到 4d（投射到高维空间提取复杂模式），第二个再降回来。注意力子层提供的主要是 Softmax 归一化，逐位置的非线性变换由 FFN 承担。

#### 残差连接（Add）

每个子层的输出加上其输入：`output = Sublayer(x) + x`。作用是让梯度能直接跨层传播，解决深层网络的训练困难（类似 ResNet 的思路）。
...
$$
PE\_{(pos,\, 2i)} = \sin\!\big(pos \,/\, 10000^{2i/d}\big)
$$

$$
PE\_{(pos,\, 2i+1)} = \cos\!\big(pos \,/\, 10000^{2i/d}\big)
$$
...
- **因果掩码（Causal Mask）**：在解码器自注意力中，将未来位置的分数设为负无穷，Softmax 后权重为 0，确保自回归特性
...
| 计算复杂度 | O(T) 顺序步 | O(T) 顺序步 + O(T²) 注意力 | O(T²) 注意力（可并行） |

Transformer 用 O(T²) 的并行计算换掉了 O(T) 的串行计算，在 GPU 上反而更快。当 T 极大时（数万 token），T² 会成为瓶颈——这催生了后续的稀疏注意力、线性注意力等优化。
...
解码器的关键：用下三角掩码确保位置 i 只能注意到 ≤ i 的位置。
